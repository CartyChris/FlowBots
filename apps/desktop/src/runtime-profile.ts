...
export const RUNTIME_MODES: RuntimeModeOption[] = [
  ...
  },
  {
    id: "full-local",
    title: "Full Local",
    description: "Connect to a full local Rakazo stack with PostgreSQL and your chosen computer provider.",
  },
  {
    id: "remote",
    title: "Remote",
    description: "Connect this desktop client to a trusted Rakazo server.",
  },
];
...
export function normalizeRuntimeProfile(value: {
  ...
  throw new Error(`Unknown Rakazo runtime mode "${value.mode}".`);
}
...
function normalizeServerUrl(value: string, mode: "full-local" | "remote"): string {
  ...
  try {
    ...
    throw new Error("Enter a valid http:// or https:// Rakazo server URL.");
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Rakazo server URLs must use http:// or https://.");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Credential-bearing URLs are not allowed. Sign in inside Rakazo instead.");
  }
  if (parsed.search || parsed.hash) {
    throw new Error("Rakazo server URLs cannot include query strings or fragments.");
  }
  if (
    mode === "remote" &&
    ...
    throw new Error("Remote Rakazo servers must use HTTPS; HTTP is allowed only for localhost.");
  }
...
<title>Choose how Rakazo runs</title>
<style>
...
<div class="eyebrow">Rakazo Runtime</div><h1>How should Rakazo run?</h1>
...
<button class="card" data-runtime-mode="full-local"><strong>Full Local</strong><p>Connect to the complete local Rakazo stack with PostgreSQL and your chosen isolated computer provider.</p></button>
<button class="card" data-runtime-mode="remote"><strong>Remote</strong><p>Connect to a trusted Rakazo server over the network.</p></button>
</div>
